...
enum JOBPOINT_TYPES
{
    ...
    JP_AFFLATUS_SOLACE_EFFECT   = JPCATEGORY_WHM + 0x08, //cure potency +2 (not a percent?)
    ...
    //COR
    JP_WILD_CARD_EFFECT         = JPCATEGORY_COR + 0x00,
    JP_CUTTING_CARDS_EFFECT     = JPCATEGORY_COR + 0x02,
    JP_PHANTOM_ROLL_DURATION    = JPCATEGORY_COR + 0x04,
    JP_BUST_EVASION             = JPCATEGORY_COR + 0x06,
    JP_QUICK_DRAW_EFFECT        = JPCATEGORY_COR + 0x08,
    JP_AMMO_CONSUMPTION         = JPCATEGORY_COR + 0x0A,
    JP_RANDOM_DEAL_EFFECT       = JPCATEGORY_COR + 0x0C,
    JP_COR_RANGED_ACC_BONUS     = JPCATEGORY_COR + 0x0E,
    JP_TRIPLE_SHOT_EFFECT       = JPCATEGORY_COR + 0x10,
    JP_OPTIMAL_RANGE_BONUS      = JPCATEGORY_COR + 0x12,

    //PUP
    JP_OVERDRIVE_EFFECT         = JPCATEGORY_PUP + 0x00,
    JP_HEADY_ARTIFICE_EFFECT    = JPCATEGORY_PUP + 0x02,
    JP_AUTOMATON_HP_MP_BONUS    = JPCATEGORY_PUP + 0x04,
    JP_ACTIVATE_EFFECT          = JPCATEGORY_PUP + 0x06,
    JP_REPAIR_EFFECT            = JPCATEGORY_PUP + 0x08,
    JP_DEUS_EX_AUTOMATA_RECAST  = JPCATEGORY_PUP + 0x0A,
    JP_TACTICAL_SWITCH_BONUS    = JPCATEGORY_PUP + 0x0C,
    JP_COOLDOWN_EFFECT          = JPCATEGORY_PUP + 0x0E,
    JP_DEACTIVATE_EFFECT        = JPCATEGORY_PUP + 0x10,
    JP_PUP_MARTIAL_ARTS_EFFECT  = JPCATEGORY_PUP + 0x12,

    //DNC
    JP_TRANCE_EFFECT            = JPCATEGORY_DNC + 0x00,
    JP_GRAND_PAS_EFFECT         = JPCATEGORY_DNC + 0x02,
    JP_STEP_DURATION            = JPCATEGORY_DNC + 0x04,
    JP_WALTZ_POTENCY_BONUS      = JPCATEGORY_DNC + 0x06,
    JP_WALTZ_POTENCY_BONUS      = JPCATEGORY_DNC + 0x08,
    JP_JIG_DURATION             = JPCATEGORY_DNC + 0x0A,
    JP_FLOURISH_I_EFFECT        = JPCATEGORY_DNC + 0x0C,
    JP_FLOURISH_II_EFFECT       = JPCATEGORY_DNC + 0x0E,
    JP_FLOURISH_III_EFFECT      = JPCATEGORY_DNC + 0x10,
    JP_CONTRADANCE_EFFECT       = JPCATEGORY_DNC + 0x12,

    //SCH
    JP_TABULA_RASA_EFFECT       = JPCATEGORY_SCH + 0x00,
    JP_CAPER_EMMISSARIUS_EFFECT = JPCATEGORY_SCH + 0x02,
    JP_LIGHT_ARTS_EFFECT        = JPCATEGORY_SCH + 0x04,
    JP_DARK_ARTS_EFFECT         = JPCATEGORY_SCH + 0x06,
    JP_STRATEGEM_EFFECT_I       = JPCATEGORY_SCH + 0x08,
    JP_STRATEGEM_EFFECT_II      = JPCATEGORY_SCH + 0x0A,
    JP_STRATEGEM_EFFECT_III     = JPCATEGORY_SCH + 0x0C,
    JP_STRATEGEM_EFFECT_IV      = JPCATEGORY_SCH + 0x0E,
    JP_MODUS_VERITAS_EFFECT     = JPCATEGORY_SCH + 0x10,
    JP_SUBLIMATION_EFFECT       = JPCATEGORY_SCH + 0x12,

    //GEO
    JP_BOLSTER_EFFECT           = JPCATEGORY_GEO + 0x00,
    JP_WIDENED_COMPASS_EFFECT   = JPCATEGORY_GEO + 0x02,
    JP_LIFE_CYCLE_EFFECT        = JPCATEGORY_GEO + 0x04,
    JP_BLAZE_OF_GLORY_EFFECT    = JPCATEGORY_GEO + 0x06,
    JP_GEO_MAGIC_ATK_BONUS      = JPCATEGORY_GEO + 0x08,
    JP_GEO_MAGIC_ACC_BONUS      = JPCATEGORY_GEO + 0x0A,
    JP_DEMATERIALIZE_DURATION   = JPCATEGORY_GEO + 0x0C,
    JP_THEURGIC_FOCUS_EFFECT    = JPCATEGORY_GEO + 0x0E,
    JP_CONCENTRIC_PULSE_EFFECT  = JPCATEGORY_GEO + 0x10,
    JP_INDI_SPELL_DURATION      = JPCATEGORY_GEO + 0x12,

    //RUN
    JP_ELEMENTAL_SFORZO_EFFECT  = JPCATEGORY_RUN + 0x00,
    JP_ODYLLIC_SUBTER_EFFECT    = JPCATEGORY_RUN + 0x02,
    JP_RUNE_ENCHANTMENT_EFFECT  = JPCATEGORY_RUN + 0x04,
    JP_VALLATION_DURATION       = JPCATEGORY_RUN + 0x06,
    JP_SWORDPLAY_EFFECT         = JPCATEGORY_RUN + 0x08,
    JP_SWIPE_EFFECT             = JPCATEGORY_RUN + 0x0A,
    JP_EMBOLDEN_EFFECT          = JPCATEGORY_RUN + 0x0C,
    JP_VIVACIOUS_PULSE_EFFECT   = JPCATEGORY_RUN + 0x0E,
    JP_ONE_FOR_ALL_DURATION     = JPCATEGORY_RUN + 0x10,
    JP_GAMBIT_DURATION          = JPCATEGORY_RUN + 0x12
};
