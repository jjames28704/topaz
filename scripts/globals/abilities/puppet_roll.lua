...
function onAbilityCheck(player,target,ability)
    local effectID = dsp.effect.PUPPET_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, dsp.effect.PUPPET_ROLL, dsp.job.PUP)
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(dsp.merit.BUST_DURATION), dsp.effect.PUPPET_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_PET_MACC) == false) then
