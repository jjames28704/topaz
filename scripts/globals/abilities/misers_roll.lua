...
function onAbilityCheck(player,target,ability)
    local effectID = dsp.effect.MISERS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, dsp.effect.MISERS_ROLL, dsp.job.COR)
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(dsp.merit.BUST_DURATION), dsp.effect.MISERS_ROLL, effectpower, 0, duration, caster:getID(), total, dsp.mod.SAVETP) == false) then
